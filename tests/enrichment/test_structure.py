...
from src.enrichment.structure import StructuralExtractionError, extract_structure
...
def test_vendor_failure_is_wrapped_with_source_and_language() -> None:
    with patch("src.enrichment.structure.process", side_effect=RuntimeError("broken")):
        with pytest.raises(
            StructuralExtractionError,
